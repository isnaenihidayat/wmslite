...
<div class='container popup' style="display: block;">
    <div class="panel panel-success">
        <div class="panel-heading">
            <h3 class="panel-title"><?php echo t("Recipient") ?></h3>
            <a href="<?php echo Yii::app()->createUrl("otr", array()) ?>" class="glyphicon glyphicon-remove" style="color: white;"></a>
        </div>
        <div class="panel-body">
            <div>
                <a class="btn btn-primary recipient_new" href="javascript:;">
                    <?php echo t("Add Recipient") ?>
                </a>
                <a class="btn btn-warning" href="<?php echo Yii::app()->createUrl("otr/recipient", array()) ?>">
                    <?php echo t("Refresh") ?>
                </a>
            </div>
            <br>
            <table id="recipient_list" class="table table-striped table-bordered table-hover" style="width: 100%;">
                <thead>
                    <tr>
                        <th><?php echo t("#") ?></th>
...
<?php
$this->renderPartial('/otr/recipient_new', array());;
$this->renderPartial('/otr/recipient_edit', array());;
$this->renderPartial('/otr/recipient_delete', array());;

$url = Yii::app()->createUrl('/ajax/recipient_list');

$js = <<<JS
$(document).ready(function() {
    var dataTable =  $('#recipient_list').DataTable( {
        "scrollY": 200,
        "scrollX": true,
        "iDisplayLength": 20,
        "bProcessing": true,
...
$(document).on("click", ".recipient_new", function () {
    $("#id").val('');
    id = $(this).data('id');

    $("#id").val(id);
    $(".recipient_new_modal").modal('show');
});

$(document).on("click", ".recipient_edit", function (e) {
    var id = $(this).data('id');
    $.ajax({
        url: 'otr/recipient_get',
        method: 'POST',
        data: 'id='+id,
    ...
    $(".recipient_edit_modal").modal('show');
});

$(document).on("click", ".recipient_delete", function (e) {
    var id = $(this).data('id');
    $.ajax({
        url: 'otr/recipient_get',
        method: 'POST',
    ...
    $(".recipient_delete_modal").modal('show');
});
