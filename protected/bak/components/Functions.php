...
class Functions extends CApplicationComponent
{
   ...
   public function jsLanguageAdmin()
    ...
    public function sendEmail($to='',$from='',$subject='',$body='')
    {
        ...
        $res='';
		if (!empty($to)) {
            $res=$this->mailSend($to,$from,$subject,$body);
            if($res=='sent'){
				$params['status']="send";
    ...
    public function mailSend($to,$from,$subject,$message){
        ...
        $mail->SetFrom($from, 'Elcar Support');
	...
	public static function generateCode($length = 8) {
	 ...
	 public static function generateCodes($length = 8) {
	...
	public static function getLogoURL()
	{
		...
		if(!empty($logo)){
			...
			} else return Yii::app()->getBaseUrl(true)."/assets/images/logo.png";
		} else return Yii::app()->getBaseUrl(true)."/assets/images/logo.png";
	}

	public static function getLogoClient()
    {
        $logo = getOptionA("client_logo");
        if(!empty($logo)){
            $logo_path=Driver::uploadPath()."/$logo";
            if(file_exists($logo_path)){
                return Driver::uploadURL()."/$logo";
            } else return Yii::app()->getBaseUrl(true)."/assets/images-front/me.png";
        } else return Yii::app()->getBaseUrl(true)."/assets/images-front/me.png";
