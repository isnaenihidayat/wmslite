...
class OtrController extends CController
{
    ...
    public function actionDashboard()
    {
        ...
        if (!empty($keyword)) {
            ...
            $sql = "SELECT utama.*,
                ...
                `h`.`hawb` AS `hawb`,
                `h`.`descr` AS `descr`,
                `h`.`delivery_id` AS `delivery_id`,
                `h`.`po` AS `po`,
                `h`.`ship_method` AS `ship_method`,
                `h`.`ata` AS `ata`,
                `h`.`sppb_date` AS `sppb_date`,
                `h`.`locator` AS `locator`,
                y.orderKey AS orderKey_ob,
                y.externOrderKey AS externOrderKey_ob,
                y.sku AS sku_ob,
                y.destination,
                y.pso_delivery_id,
                y.transporter
                FROM (
                SELECT id.externReceiptKey, id.lottable07, id.toLot, p.lot, p.orderKey, id.sku
                FROM el_schenker_inbound_detail id
                LEFT JOIN el_schenker_outbound_pick p ON p.lot=id.toLot
                ) AS utama
                INNER JOIN el_inbound_header h ON h.delivery_id=utama.externReceiptKey AND h.warehouse='marunda' AND h.hawb=utama.lottable07
                LEFT JOIN (
                SELECT p.lot, od.orderKey, od.lottable07, od.sku, od.externOrderKey,
                (SELECT ccompany FROM el_schenker_outbound_header WHERE orderKey=p.orderKey) AS destination,
                (SELECT buyerPO FROM el_schenker_outbound_header WHERE orderKey=p.orderKey) AS buyerPO,
                (SELECT susr1 FROM el_schenker_outbound_header WHERE orderKey=p.orderKey) AS pso_delivery_id,
                (SELECT carrierName FROM el_schenker_outbound_header WHERE orderKey=p.orderKey) AS transporter
                FROM el_schenker_outbound_pick p INNER JOIN el_schenker_outbound_detail od ON od.orderKey=p.orderKey 
                ) AS y ON y.lot=utama.lot AND y.sku=utama.sku AND y.lottable07=utama.lottable07
                WHERE 
                h.hawb LIKE '%" . $keyword . "%' 
                OR h.locator LIKE '%" . $keyword . "%' 
                OR utama.sku LIKE '%" . $keyword . "%' 
                OR h.delivery_id LIKE '%" . $keyword . "%' 
                OR y.pso_delivery_id LIKE '%" . $keyword . "%' 
                OR externReceiptKey LIKE '%" . $keyword . "%' 
                OR externOrderKey LIKE '%" . $keyword . "%'";
                ...
                // SELECT externReceiptKey, hawb AS lottable07, 
                // NULL AS toLot,
                //   NULL AS lot,
                //   NULL AS orderKey,
                //     descr AS sku,  
                //     id_ih AS id,
                //     hawb AS hawb,
                //     descr_detail AS descr,
                //     externReceiptKey AS delivery_id,
                //     po_in AS po,
                //     ship_method AS ship_method,
                //   ata AS ata,
                //   sppb_date AS sppb_date,
                //   locator AS locator,
                //     NULL AS orderKey_ob,
                //   po_out AS externOrderKey_ob,
                //   descr AS sku_ob,
                //   destination AS destination,
                //   delivery_id AS pso_delivery_id,
                //   transporter AS transporter
                //     FROM (
                    
                // SELECT ih.id AS id_ih, ih.hawb, id.descr, ih.descr AS descr_detail, ih.delivery_id AS externReceiptKey,
                //             id.loc AS loc_inbound, oh.po AS po_out, ih.po AS po_in, ship_method, ata, sppb_date,
                //             locator, od.loc, 
                ...
                //             LEFT JOIN el_outbound_details od ON od.descr=id.descr AND od.hawb=id.hawb
                //             LEFT JOIN el_outbound_header oh ON oh.id=od.id
                //             WHERE ih.warehouse='arcadia' AND ih.delivery_id=id.sso_delivery_id AND 
                //             ih.hawb LIKE '%" . $keyword . "%'
                //             OR id.descr = '" . $keyword . "'
                //             OR ih.locator LIKE '%" . $keyword . "%'
                //             OR ih.po LIKE '%" . $keyword . "%'
                //                         ) AS x2;
  ...
  LEFT JOIN (
  SELECT p.lot, od.orderKey, od.lottable07, od.sku, od.externOrderKey,
  ccompany AS destination,
  buyerPO AS buyerPO,
  susr1 AS pso_delivery_id,
  carrierName AS transporter
  FROM el_schenker_outbound_pick p INNER JOIN el_schenker_outbound_detail od ON od.orderKey=p.orderKey 
  LEFT JOIN el_schenker_outbound_header oh ON oh.orderKey=p.orderKey
  ) AS y ON y.lot=utama.lot AND y.sku=utama.sku AND y.lottable07=utama.lottable07
  WHERE 
    h.hawb LIKE '%" . $keyword . "%' 
    OR h.locator LIKE '%" . $keyword . "%' 
...
";
            ...
            //     SELECT 
            //     h.delivery_id AS externReceiptKey,
            //     h.hawb AS lottable07,
            //     NULL AS toLot,
            //     NULL AS lot,
            //     NULL AS orderKey,
            //     d.descr AS sku,
            ...
            //     h.locator AS locator,
            //     NULL AS orderKey_ob,
            //     NULL AS externOrderKey_ob,
            //     NULL AS sku_ob,
            //     NULL AS destination,
            //     NULL AS pso_delivery_id,
            //     NULL AS transporter

            //     FROM el_inbound_header h 
            //     LEFT JOIN el_inbound_details d ON d.sso_delivery_id= h.delivery_id 
            //     WHERE h.warehouse='marunda' AND h.status <> 'successful'
            //     AND 
            //     h.hawb LIKE '%" . $keyword . "%' 
            //       OR h.locator LIKE '%" . $keyword . "%' 
            //       OR d.descr = '". $keyword . "' 
            ...
            $shipment_search = Yii::app()->db->createCommand($sql2)->queryAll();
        }
    ...
    public function actionPushOutbound()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                $transaction = Yii::app()->db->beginTransaction();

                $ib = Yii::app()->db->createCommand()
                    ->select('*')
                    ->from('el_inbound_header')
                    ->where('id=:id', array(':id' => $_POST['pusho_id_inbound']))
                    ->queryRow();

                if ($ib) {
                    Yii::app()->db->createCommand()->insert('el_schenker_inbound_header', [
                        'susr1' => $ib['modality'],
                        'externReceiptKey' => $ib['delivery_id'],
                        'receiptDate' => $ib['ata'],
                        'actualShipDate' => date('d/m/Y H:i:s', strtotime($ib['ata'])),
                        'created_at' => date('Y-m-d H:i:s'),
                    ]);

                    $ibschh = Yii::app()->db->getLastInsertID();

                    Yii::app()->db->createCommand()->insert('el_schenker_inbound_detail', [
                        'header_id' => $ibschh,
                        'lottable07' => $ib['hawb'],
                        'sku' => $ib['hawb'],
                        'toLot' => $ib['hawb'],
                    ...
                    Yii::app()->db->createCommand()->insert('el_schenker_outbound_header', [
                        'orderKey' => $ib['hawb'],
                        'ccompany' => $_POST['pusho_destination'],
                        'carrierName' => $_POST['pusho_transporter'],
                        'externOrderKey' => $_POST['pusho_gon_number'],
                        'actualShipDate' => date('d/m/Y H:i:s', strtotime($_POST['pusho_ship_date'])),
                        'created_at' => date('Y-m-d H:i:s'),
                    ]);

                    $id = Yii::app()->db->getLastInsertID();

                    Yii::app()->db->createCommand()->insert('el_schenker_outbound_detail', [
                        'header_id' => $id,
                        'orderKey' => $ib['hawb'],
                        'susr1' => $_POST['pusho_pso_delivery_id'],
                        'sku' => $ib['hawb'],
                        'externOrderKey' => $_POST['pusho_gon_number'],
                        'lottable07' => $_POST['pusho_hawb'],
                        'created_at' => date('Y-m-d H:i:s'),
                    ]);

                    Yii::app()->db->createCommand()->insert('el_schenker_outbound_pick', [
                        'orderKey' => $ib['hawb'],
                        'lot' => $ib['hawb'],
                        'created_at' => date('Y-m-d H:i:s'),
                    ]);                    

                    Yii::app()->db->createCommand()->update('el_inbound_header', array(
                        'shipment_only' => 1,
                        'status' => 'successful',
                        'date_updated' => date('Y-m-d H:i:s'),
                    ), 'id=:id', array(':id' => $_POST['pusho_id_inbound']));

                    // 

                    $transaction->commit();

                    Yii::app()->user->setFlash('success', "Push Outbound has been successfully added");
                }
                ...
                Yii::app()->user->setFlash('danger', "Failed when push outbound, transaction rollback");
            }
        }
    }
